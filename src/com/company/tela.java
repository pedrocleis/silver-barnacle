...
public class tela extends javax.swing.JFrame {
    ...
    private void btnFiltroSobelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFiltroSobelActionPerformed
        ...
        int[][] sobelVerticalRebatido = {{1, 0, -1}, {2, 0, -2}, {1, 0, -1}};
        int[][] sobelHorizontalRebatido = {{-1, -2, -1}, {0, 0, 0}, {1, 1, 1}};
        dstImage = midImage;
        for (int i = (size / 2); i < width - (size / 2); i++) {
            for (int j = (size / 2); j < height - (size / 2); j++) {
                dstImage[0][i][j] = 0;
                ...
                for (int matrixW = -(size / 2); matrixW <= (size / 2); matrixW++) {
                    for (int matrixH = -(size / 2); matrixH <= (size / 2); matrixH++) {
                        ...
                        dstImage[0][i][j] += Math.round(midImage[0][i - matrixW][j-matrixH] * sobelVerticalRebatido[matrixW+1][matrixH+1]);

                        dstImage[1][i][j] += Math.round(midImage[1][i - matrixW][j-matrixH] * sobelVerticalRebatido[matrixW+1][matrixH+1]);
                        dstImage[2][i][j] += Math.round(midImage[2][i - matrixW][j-matrixH] * sobelVerticalRebatido[matrixW+1][matrixH+1]);
